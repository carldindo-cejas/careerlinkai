...
| `[~]` P4-19 | **`csp-check.mjs`'s admin leg was single-use per database** — script fixed 2026-08-02, see below; the 9 authenticated screens have still not been *observed* green, because re-arming the local accounts needs a credential write | P4-15 finding | 1–2 h |

### `[~]` **P4-19 — `csp-check.mjs` could only ever pass once** — *script fixed 2026-08-02. The nine screens remain unobserved.*
...
**Verified in its failure path only, and that distinction is the point.** The rewritten leg
correctly identifies an already-cleared gate and says `wrong password, or the account is not
active` instead of timing out against a `/change-password` that will never arrive, and the new
coverage assertion fires with the sentence above. What has **not** been observed is the leg
*passing* and the nine screens coming back green.

**Why it is still open, stated plainly.** The local preview database has three staff rows
(`admin@` and two counselors) and nobody knows the admin's current password: it was rotated through
the UI, which clears `must_change_password`, so `bootstrap-staff.mjs --local` is the only way back
to a known credential. The production accounts are a **different database** — `cejascarldindo@`
exists there and does not exist locally — and `csp-check.mjs` refuses to run against production by
design, correctly: the CSP is a property of `_headers`, which is byte-identical in both, so there
is nothing production offers except the risk of rotating a live staff password.

* **Files:** `scripts/csp-check.mjs`
* **Verify:** ⏳ `cd backend && npm run preview`, then
  `node scripts/bootstrap-staff.mjs --database CareerLinkAI_Main --local --password ChangeMe123`,
  then `node scripts/csp-check.mjs --password ChangeMe123` → expect **PASS, 15 screens**. The
  middle step writes a staff credential and has not been run.
