...
| `[~]` P4-2 | **Email delivery via Resend** — password-reset half ✅ done 2026-08-01 (code path proven by tests + 4 mutations; delivery **not yet witnessed** — domain unverified, no key set). Notification half **deliberately not built**: §63's trigger for it has not fired. Introduces the system's only secret | D7 (Medium) | 2–3 d |
...
```
        ...
        P4-2  password-reset email (Resend)      ✅ code done — Cloudflare's paid plan was ruled out
                                                    by §45 and its free path by measurement; nothing
                                                    is delivered until the domain is verified
NOW ──► Phase 4  (+ three dashboard-side steps: DEPLOYMENT.md §8.1 WAF rule, §8.2 Workers Logs
        alert, §8.3 Resend domain + API key — §8.3 turns P4-2 from written into working)
```
