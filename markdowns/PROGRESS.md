...
- **The program composite was re-weighted on 2026-09-18 (deviation D34) so that programs follow
  the careers.** §27's weights are no longer what ships; `PROGRESS` records why in D34 and
  `lib/recommendation.ts`'s header carries the full table. §28's program worked example is
  consequently **72.0**, re-derived by hand, not 76.1 — the career half (69.1) did not move.
...
| D10 | **Staff tokens expire after 7 days** (`STAFF_TOKEN_TTL_HOURS`, default `168`, a `[vars]` entry read via `lib/config.ts` — pre-production audit L4 moved it off a hardcoded `24 * 7` constant) | `docs/api/phase-1` §"Token lifecycle" states staff tokens expire **"never — until logout"**; FULLPLAN §38 pins an expiry only for *students* | **Accept — the port is stricter than the contract, deliberately.** A never-expiring bearer token on a counselor account that can read every student's results is a worse default than a week-long one, and the frontend already signs itself out cleanly on any 401 (`httpClient` interceptor), so a lapsed token degrades to "log in again" rather than to a broken screen. Flagged here rather than silently: if the 7-day window annoys counselors in practice, edit the var — do not remove it. |
...
| **D25** | **Pre-production audit remediation (Phases A–H)** — a hardening pass over the backend, tracked in `BACKENDAUDIT.md`, that touched no engine, schema table, or route contract | The audit is not FULLPLAN; this row records that the plan's implementation was audited and hardened before the defense | **Done — see the "Pre-Production Audit Remediation" section below.** The two Critical findings (C1 results-listing subrequest blowup, C2 non-atomic submit) and the High/Medium/Low findings behind them are all resolved, each phase deployed to staging and left with the suite green (533 tests). |
| **D26** | **The assessment taxonomy's reference rows ship inside migration 0014**, not in `seeds/` — 12 `assessment_types`, 15 `assessment_scorings`, and the 80-row `assessment_type_scorings` compatibility matrix | The house convention (seeds/0001–0003) puts seeded content in `seeds/*.sql`, applied by an `npm run db:seed:*` script separate from the migration | **Ratified, and the distinction is the point.** Everything in `seeds/` is *demo content* — five universities, a roster — that an environment may legitimately be without. These rows are not content: `assessment_type_scorings` is **server-side validation input** (`AssessmentTaxonomyService.assertCompatible` reads it on every create and edit), so an environment missing it would reject every assessment ever saved rather than merely look empty. `test/setup.ts` applies **migrations only**, so a seeds-based taxonomy would also be invisible to the suite that tests the rule. Reference data that validation reads belongs where the CHECK constraints already are. Every insert is `INSERT OR IGNORE` against fixed UUIDs, so re-running is a no-op. |
| **D27** | **A fifth domain event, `ClassActivated`** (v1.6; `ClassCreated` until then) (`src/events/dispatcher.ts`), consumed by one listener — `applyGlobalAssignments`, which gives a class every assessment currently assigned globally | §60 catalogs exactly **four** events, and class creation is not one of them | **Accept, with the reason stated.** Migration 0014's global assignment is one ordinary `assessment_assignments` row *per class* — deliberately, so enrollment, `canStartAttempt`, `authorizeViewAttempt`, the results join and the §44 fan-out all keep resolving through a real class (a nullable `class_id` meaning "everyone" would make `classForAttempt` return NULL, and that guards the most sensitive data in the system, §40). The cost of that choice is one gap: a class created *after* the assignment has no row, so "Global" would be quietly false. Something must therefore react to class creation, and the only two arrangements are a direct call or an event. A direct call would put an assessment concern inside `ClassService.create`, breach §11 (a module may not query another module's tables), **and close an import cycle** — `AssessmentAttemptService` already imports `ClassService`. The event does none of that and gets the failure mode right for free: `dispatch()` logs and absorbs a throwing listener, so a class is still created if the rollout fails, and an admin re-running "Assign globally" is the recovery. No notification fires from this path and none is missing — a class has no roster at creation. | **Extended in v1.6:** the event now carries `reason: 'CREATED' | 'REACTIVATED'` and fires on the *second* way a class becomes eligible — a `draft` or `archived` class switched to `active`. That was a real hole rather than a tidy-up: `assignToClasses` targets `listActive()`, so a class drafted before a global assignment and activated afterwards never received it, and its students were permanently missing an assessment the admin list truthfully reported as reaching “every class”. One fact with two causes is one event; the reason travels so the audit row does not claim a class was created when it was switched on. Unlike creation, reactivation **does** notify — a class being switched back on may already hold a roster, and handing forty students an assessment in silence is how a deadline is missed.
| **D28** | **`assessment_templates` gains a real Delete** (soft, `DELETE /assessment-templates/{id}`), alongside Archive | §20 catalogs no delete for assessments; §12 lists `assessment_templates` as soft-deletable but nothing ever set the column | **Accept — the column existed, the act did not.** §12 put `assessment_templates` on the soft-delete list and `deleted_at` has been filtered on by every list in the module since Phase 3, but no code path ever wrote it — so an assessment created by mistake could only be archived, cluttering the assignable list forever with no way to remove it. The delete is soft rather than hard for the reason §12 gives: every FK beneath a template cascades, so a hard `DELETE` would silently take the authoring history with it. Two guards, refused as a **422 with the reason** rather than a 403: an assessment with any attempt against it can never be deleted (§12 — the attempt → answer → result chain is permanent evidence, and removing the instrument would leave every row pointing at a title nobody can read), and one with an open assignment must have those closed first. Migration 0016 adds `deleted_by`: `deleted_at` recorded *when*, and nothing recorded *who*. |
| **D29** | **`assessment_versions.published_at`** (migration 0016) | Nothing in FULLPLAN carries a publication date; §13.4 lists `created_at` only | **Forced by the requirement, and `created_at` could not stand in.** The administrator's table must show and filter on Date Published, and publishing was a status flip that recorded no time — `created_at` is when the version was *drafted*, which for an instrument that sat in review for three weeks is a different date entirely. The template deliberately has **no** publish column: publication happens to a version, so the assessment's date is derived (`MAX(published_at)` across its versions) rather than denormalized, because a denormalized copy has to be maintained by every path that publishes and the one that forgets is silently wrong forever. NULL means *never published* — a state, not missing data — and sorts last in both directions. The backfill sets existing PUBLISHED versions to `created_at`: a lower bound rather than a guess. |
| **D30** | **The audit viewer filters by action *group*** (`action_type`: CREATE/UPDATE/DELETE/…), resolved from an exhaustive `Record<AuditAction, AuditActionType>` | §20 documents `action`, `module`, `user_id` and a date range only | **Accept.** An operator thinks “show me everything that was deleted”, not in the fifty-value action vocabulary. The mapping is an exhaustive record rather than a suffix pattern, and that is the whole reason it is trustworthy: a `LIKE '%_DELETED'` heuristic silently misfiles `ROSTER_STUDENT_REMOVED` and `PROGRAM_CAREER_UNLINKED` — both deletions — and keeps misfiling every action added afterwards, whereas a missing entry in the record is a **type error**. It also turns the filter into an `IN (…)` over an indexed column rather than a leading-wildcard scan, which on a table with no retention policy (§13.8) is the difference between a seek and a full scan. The CSV export is capped at 5,000 rows and says so: an unbounded export on an unbounded table works in testing and fails at whatever size the log has reached in production, which is the worst failure mode available. |
| **D31** | **`lib/d1-batching.ts`** — the D1 parameter budget derived from the table via `getTableColumns`, replacing three hand-counted widths | D18 established the chunking; nothing says where the width comes from | **Forced by a live bug this work surfaced.** `assessment_assignments` was chunked at a literal `7` in two files. Migration 0014 added `scope`, making it 8, and neither copy was updated — so a global assignment bound 14 × 8 = **112** parameters per statement against D1's 100-parameter ceiling. It failed only when a single act targeted **thirteen or more classes**, so it passed every test and every small deployment while being broken at exactly the scale the feature was added for; `test/assessment/lifecycle.test.ts` is what finally accumulated enough classes to hit it. `chunkForInsert(rows, table)` takes the table rather than a number, so adding a column updates the arithmetic and there is nothing left to forget. |
| **D32** | **`student_profiles` serialization carries a second completeness answer**, `profiling.{is_complete, missing}`, broader than `is_complete_for_recommendations` | §27 needs `strand` and `gwa`; nothing defines a wider “profile complete” | **Two questions, two answers — collapsing them would break one.** `is_complete_for_recommendations` is a statement about the **engine**: strand and GWA are the two inputs §27 cannot run without, and it has meant exactly that since Phase 3 (a test pins the pair). `profiling` is a statement about the **student**: what is still missing from the profile they were asked to fill in, which additionally includes grade level — §27 never reads it, while the counselor's roster and every result export do. Loosening the engine's precondition to match, or telling a student their profile is incomplete for a reason that is not true, are both worse than carrying both facts. The persistent dashboard banner renders from `profiling.is_complete` and from nothing else, which is what makes “it disappears once profiling is done” true by construction rather than by a second rule that could drift. |
| **D33** | **The builder gained per-question `PATCH`/duplicate/delete and a version-level reorder**; the old read-only review list plus bottom-of-page “add a question” form is gone | §31 describes review-and-edit of *text and options*; §20 catalogs `PATCH /assessment-questions/{id}` for text and `required` only | **Accept — the old surface could not do what §31 asked.** A question's type, options and scoring mapping were fixed at creation, items could not be reordered, and confirming a mapping meant hunting for the item it belonged to. Every new path enforces the same rules the old one did, in the same place: invariant 1 refuses all four on a non-DRAFT version (a test asserts each), a mapping the author types is confirmed at write time because they are the human §25 asks for, and there is still **no bulk confirm** (§31 forbids exactly that). Options and mappings are replaced wholesale rather than diffed — a diff has to decide what a “changed” option is and gets it wrong for two options swapped with different scores — which is safe precisely because a DRAFT version has no attempts. Reorder takes the **whole** order and refuses a partial list: wherever the unlisted items were put would be a change the author did not ask for. |
| **D34** | **The program composite's weights, and its components** (2026-09-18, prompt-driven): RIASEC fit 35%, **career alignment 25% (new)**, SCCT career confidence 20%, academic fit 10%, strand alignment 10%. `program_eligibility` and the program-side `student_preference` term are **removed** | §27 pins RIASEC 35%, academic fit 20%, confidence 15%, strand 15%, eligibility 10%, preference 5%, and gives a program no career-alignment term at all | **Accept — §27's weights produced an answer a student could see was wrong.** A student whose #1 career was Clinical Psychologist was shown BS Psychology at **#7**, below four programs leading nowhere near it: 30% of the composite went to `academicFit` + `programEligibility`, which are two readings of the *same* subject average and are therefore identical across every program a given student is scored against — they discriminate between none of them — while the only term reading the program's careers was an **average** that punishes a degree with one perfect destination and several ordinary ones. `careerAlignment` scores the best three linked careers with `careerMatchScore`, the same function behind the student's own career list, so "this program aligns with your careers" and "this program leads to careers near the top of your list" are one statement. The dropped `studentPreference` was a **constant 70** for every program — it changed no ranking, ever, and dropping it is what makes the five remaining weights sum to 1.00; `CAREER_WEIGHTS` still carries it for §63. Verified against production on 2026-09-18: BS Psychology moved **#7 → #2**, and two further students (a Realistic/Conventional profile and an Artistic/Social one) kept their program orderings **unchanged** — the re-weighting corrected the broken case without disturbing the ones that already worked. §28's program example is re-derived by hand to **72.0**. |

---

## Pre-Production Audit Remediation (Phases A–H)

`BACKENDAUDIT.md` is the full audit; this is the one-paragraph-per-phase record of what shipped, in
priority order. Every phase left the hermetic suite green and the frontend API contract byte-compatible.

- **A — Critical (C1, C2).** The counselor/student results listings fanned out ~5 D1 queries *per
  scored attempt* and blew the Free-plan 50-subrequest ceiling on any real class (the demo-killer,
  masquerading as a CORS error). Rewrote them as fixed-cost set queries (`hydrateResultViews`) and
  paginated them; the subrequest-budget test now covers both (≤15 D1 calls at N=40, was ~400). Made
  submit atomic — the status transition folded into scoring's single batch — so a mid-flight failure
  leaves the attempt `IN_PROGRESS` and re-submittable instead of stranded in `SUBMITTED`.
- **B — Architecture (H4, M1, L1).** A shared `translateUniqueViolation` turns lost check-then-insert
  races into 422s instead of raw 500s (saveAnswer became an atomic upsert; start recovers
  idempotently; counselor-create, roster-confirm and dimension-add translate); an atomic
  `AuthGuardDO.charge()` closed the AI-limiter TOCTOU; dead code removed. (The Phase G race tests later
  caught that `isUniqueViolation` only read `error.message` while Drizzle 0.45 puts the SQLite text on
  `.cause` — fixed to walk the cause chain.)
- **C — Database (L2, M4).** Migration `0010` adds the `audit_logs (module, created_at)` index the
  §54 viewer needs; `generateFor` now always clears the student's prior recommendations (scoped to the
  student, not one result) so a retake replaces rather than accumulates.
- **D — Performance (H2, H5, M5).** Dropped the hot-path `touchToken` write (a per-request D1 write to
  a column nothing reads); batched the last per-row query fans (assignments list, template list);
  tidied `embedBatch` and guarded its COMPLETED flip against a double-fire.
- **E — Security (H3, M2, M3, M9).** Login now derives against a constant dummy hash for unknown
  emails (no timing oracle); forgot-password is throttled per-email; the throttled-join audit row is
  written once per window, not per attempt; the reset-token compare is constant-time.
- **F — API polish (M6, M7, L4, L3).** The audit viewer accepts date-only `from`/`to`; the staff
  token TTL is a `[vars]` entry (L4, see D10); `/health` keeps `environment` by decision (L3);
  results pagination landed in A, and the bounded assignment lists stay arrays (now O(1) after H5).
- **G — Testing.** Added the crash-recovery, concurrency-race, H5-budget and queue-consumer tests the
  audit asked for — the suite that "passed 514 and died on staging" loses its remaining habitats.
- **H — Operational readiness (H1, M11, observability).** Dead-letter queues on both consumers (+ a
  DLQ consumer that records rather than drops); `[observability]` enabled in all three scopes; a
  nightly Cron Trigger (`scheduled` handler) that sweeps expired tokens and stale reset tokens; two
  new platform gates assert the DLQ/observability/cron config so a deploy cannot ship without them.
