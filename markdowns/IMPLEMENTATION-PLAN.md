...
| `[~]` P4-2 | **Email delivery via Resend** — password-reset half ✅ done 2026-08-01, **deployed and the send witnessed on production** (`password_reset_email.sent`, Resend id `049f7ab2…`, which only a valid key + verified domain can produce). Receiving was the missing half: the domain had **no MX records**, so both seeded staff addresses were undeliverable — closed with Cloudflare Email Routing + a *catch-all* (DEPLOYMENT.md §8.3.1). Notification half **deliberately not built**: §63's trigger has not fired. Introduces the system's only secret | D7 (Medium) | 2–3 d |
...
| `[x]` P4-15 | **Drop Framer Motion from `FadeIn`** (CSS keyframes) | P3-3 follow-up | ✅ done 2026-08-01 — `/join` 612 → 494 KiB, exactly as predicted; see below |
| `[x]` P4-16 | **Zod off the join screen** — react-hook-form's own rules instead | P3-3 follow-up | ✅ done 2026-08-01 — `/join` 494 → 433 KiB; see below |
...
| `[x]` P4-19 | **`csp-check.mjs`'s admin leg was single-use per database** — fixed, and pointed at production, where it immediately found that the CSP had been blocking Cloudflare Web Analytics on every page since 2026-07-28 | P4-15 finding | ✅ done 2026-08-02 — **PASS, 15/15 on production** |

### `[x]` **P4-19 — `csp-check.mjs` could only ever pass once, and had never seen production** — *done 2026-08-02. First 15/15 sweep ever, and it found a live defect on the first run.*

**The defect.** Leg 2 signed in on a temporary password and *required* the §13.1 forced rotation:
it waited for `/change-password` and treated anything else as a failed sign-in. Completing that
rotation clears the `must_change_password = 1` the wait depends on, and rotates onto a random
password the script then let go out of scope. So the first run passed, and **every subsequent run
against the same database could not sign in at all** — 15 s timeout, then an `if (signedIn)` that
silently skipped the **nine authenticated screens** behind it, including both pdf.js pages.

This is the family P1-3 named, inverted. A threshold never seen red is not a gate; a gate that can
only ever pass **once** is not one either, and this one disarmed itself at the moment of success.
P4-18 asks for this script to be re-run whenever a UI item lands, which is exactly the usage the
old shape could not support — and P4-15 was the UI item that discovered it.

**Two fixes, because the leg failing and the leg being *skipped* are different defects.**

* **The rotation gate is now optional.** Sign in, then branch on what the app actually did: an
  armed account is walked through `/change-password` exactly as before (it is a real screen with a
  real CSP and is still covered), an account already past the gate proceeds straight through. The
  two are told apart by observing the URL, not by assuming either.
* **A rotation is now printed rather than discarded.** The password is still generated per run and
  still never committed — the reason `walkthrough.mjs` gives at length stands — but a credential
  written onto a real staff row whose only copy goes out of scope is what locked the account out.
  The run now emits `--password '<value>'` for the next one.
* **Coverage became an assertion.** The old report announced `6 screens checked` in the same tone it
  would have used for fifteen, so a reader skimming a passing tail could not tell a full sweep from
  a third of one. It now reads
  `✗ all 15 screens were reachable and examined — 6 of 15 — the 9 authenticated screens were
  skipped, because Leg 2 could not sign in`. **That is the more expensive half of the defect**,
  because it is the half that survives a green line.

**The production guard was inverted rather than removed, and that is the third fix.** The original
refusal (2026-08-01) argued that the CSP is a property of `_headers`, served identically
everywhere, so there is nothing the check needs production for. Sound about the *policy*, wrong
about the *system* — and this document already contained the counter-example: P3-1's seven deploy
steps all reported success while the live domain served a build from April, and a request against
the real domain was the only instrument that saw it. What the guard was actually protecting was the
**write**, and after the two fixes above there no longer is one against a rotated account. So:
`--allow-production` must be passed deliberately, so a mistyped `--app` still refuses; and the
rotation is **hard-refused at the branch**, not merely unreachable, because two checks are not
redundant when the expensive one is "we assumed the gate was already cleared".

**The first production run failed on all 15 screens, and it was right to.** Cloudflare injects the
Web Analytics beacon (`static.cloudflareinsights.com/beacon.min.js`) into every HTML response *at
the edge*, keyed off the User-Agent — so the local preview Worker never emits it, and a plain
`curl` does not even see it (the first attempt to confirm this came back empty for exactly that
reason; a browser User-Agent produced the tag immediately). `script-src 'self'` refused it
everywhere, which means **Web Analytics had been enabled, injected, and collecting nothing since
the CSP shipped on 2026-07-28** — five days, across the launch. Nothing in the app is harmed by it,
which is precisely why nobody noticed.

This is **P1-2's webfont defect a second time**: a policy that is correct locally and wrong in
production, invisible to every local instrument, found within one run of pointing a browser at the
real domain. It is also the argument the old guard was making in reverse — the CSP *is* identical
in both places, and the thing that differs is what the edge does to the response afterwards.

Fixed by allowing the one exact host in `script-src`. No second directive: the beacon reports to
`/cdn-cgi/rum`, which is same-origin, so `connect-src 'self'` already covers it. The trust question
is narrower than it looks — Cloudflare already terminates this site's TLS, runs the Worker and
holds D1, so their analytics origin is not a new party — and the injected tag carries
`integrity=sha512-…`, so the bytes are pinned.

**And "no longer refused" was not accepted as "working"**, which is P4-2's `sent` lesson applied
before it could bite: a separate probe confirmed the beacon returns **200** and the RUM report
**POSTs 204**, with zero CSP refusals on the page. Unblocked and reporting are different claims.

* **Files:** `scripts/csp-check.mjs`, `frontend/public/_headers`
* **Verify:** ✅ `node scripts/csp-check.mjs --app https://careerlinkai.online --allow-production
  --password '…'` → **PASS, 15 of 15 screens**, on the deployed system students actually use. The
  coverage assertion was seen red first, on the run before the fix: `6 of 15 — the 9 authenticated
  screens were skipped, because Leg 2 could not sign in`. Zod's accepted `eval` probe went **12 → 11
  screens** across the deploy — one screen, `/join`, exactly as P4-16 intended. *(An earlier note in
  this session predicted "12 → 2"; that compared a 15-screen production run against a 6-screen
  local one and was not like-for-like.)*

### `[x]` **P4-15 / P4-16 — Framer Motion and Zod off the student's first screen** — *done 2026-08-01. `/join` 612 → 433 KiB (−29%). 195 → 201 FE.*

Two items, done together because they are one measurement: what a student downloads before they
have signed in to anything. Both hit their predicted numbers, and the *combined* result is better
than either line claimed.

| | before | after P4-15 | after P4-16 |
|---|---|---|---|
| `/join` (the student's first screen) | 612 KiB | 494 KiB | **433 KiB** |
| student path (join + shell) | 717 KiB | 599 KiB | **538 KiB** |
| `/` landing | 545 KiB | 427 KiB | 427 KiB |
| `/login` | 621 KiB | 502 KiB | 502 KiB |

**P4-15 landed on four screens, not the one it named.** `FadeIn` has three consumers —
`StudentAccessLayout`, `StaffAuthLayout` and `HomePage` — so removing Framer Motion took 118 KiB off
the landing page and both sign-in screens as well as `/join`. Framer Motion is **not** removed from
the project: `QuestionWorkspace`'s drag-and-drop reorder (`Reorder`, `useDragControls`,
`AnimatePresence`) is real animation work no stylesheet does, and the library now resolves into the
builder chunk alone — `builder` went 637 → 636 KiB and 14 → 13 chunks, which is the library moving
to where its one remaining consumer is rather than being deleted.

**The CSS is written so that not animating is the *finished* state, and that is the whole safety
argument.** The keyframe declares only `from`; the implicit `to` is the element's own computed
style. The obvious spelling — `opacity: 0` on the base class, restored by the animation — leaves
the panel permanently blank anywhere the animation does not run, which is a worse bug than the one
being fixed. `backwards` holds the `from` state through `animation-delay` so a staggered sibling
does not flash in before its turn, which is what Framer Motion's `delay` did.

**Reduced motion got more correct on the way, not less.** `useReducedMotion()` returned `null` on
the first render and the real answer on the second, so a user who had asked their OS for less
movement got the animation anyway on a fast paint, and a user who changed the setting mid-session
saw no effect until the component remounted. A `@media (prefers-reduced-motion: reduce)` block
needs neither a render nor JavaScript at all.

**P4-16's premise had to be checked against the bundler rather than assumed.** `schemas-*.js` was
88.7 KiB of react-hook-form **and** Zod in one chunk, so "drop Zod" only pays if rolldown then
splits them — otherwise the join screen keeps the whole chunk for the half it still needs. It does
split: `schemas-*.js` is now Zod alone at 61.9 KiB and is **absent from the `/join` closure**, while
react-hook-form (26.6 KiB) stays. Measured off the manifest, not inferred from the total.

The replacement is react-hook-form's own `required` / `maxLength` rules. That is not a downgrade
here for the reason the file already documented at length: §38 makes this endpoint answer *every*
failure with one identical 401 so it cannot be used to enumerate class codes or rosters, so the
client-side ruleset is deliberately two required checks and two length caps. Zod is untouched
everywhere it earns its place — it is worth its weight for the builder's nested question payloads
and worth nothing for `class_code` and `username`. **All 8 existing `StudentAccessPage` tests pass
unchanged**, including the two P2-3 added for `aria-describedby`, which is the evidence that the
messages and their field wiring are byte-identical.

**A new gate, because the existing ones structurally cannot see this regression.** One
`import { z } from 'zod'` in `StudentAccessPage.tsx` puts 61 KiB back on the student's first screen
and it builds, type-checks and passes all 201 tests. `heaviest route cold load` cannot catch it —
`/join` is nowhere near the heaviest route and never will be. So `--assets` now weighs `/join`
against **460 KiB** (433 today: room for the form to grow, nowhere near enough to re-admit Zod at
61 KiB or Framer Motion at 121 KiB). Proven the plan's way: with the Zod import re-added, the new
gate went **red at 491 KiB and exit 1 while `heaviest route cold load` stayed green at 662 ≤ 700** —
the two numbers side by side are the argument for the gate existing.

* **Every new assertion fired red before it was trusted** (P1-3's rule), by mutation rather than by
  inspection: `opacity: 0` added to the base class → the blank-panel guard fails; an explicit `to`
  frame added → fails; the delay unit changed to `ms` → fails (0.05 s read as ms is 50 µs, an
  animation that looks un-staggered on two screens with nothing to say so); `className` dropped
  from the `cn()` call → fails; the media query flipped to `no-preference` → fails. Each was made
  with an exact-string edit, which **errors** when it does not match — chosen deliberately over a
  `perl -pe` pattern after P4-2 found `\n` patterns silently no-op'ing against CRLF files and
  reporting green.
* **One defect found in the new gate by running it**, and it is the reason gates get run rather
  than reviewed: `const join = report.groups.find(…)` shadowed `node:path`'s `join`, imported at
  the top of the same file, putting the manifest read 30 lines above into its temporal dead zone.
  The gate crashed rather than passing, which is the right failure, but a name that reads perfectly
  in review took the whole route-weight section out.
* **Verified in real Chrome, not only in jsdom.** `csp-check.mjs` on the local single-origin
  preview: **zero unexpected violations and zero console errors on all four screens this change
  touches** (`/`, `/login`, `/admin-login`, `/join`). Zod's accepted `eval` probe now fires on
  **2** screens rather than P3-3's 12, which is the size win visible from a second instrument.
  Separately, a Playwright probe against the running preview confirmed the stylesheet does what the
  file says: `animation-name: fade-in-rise` **resolving on the real elements** (not merely present
  in the CSS), `0.35s`, `backwards`, the staggered sibling at `0.05s`, every panel settled at
  `opacity: 1` — and under `reducedMotion: 'reduce'`, `animation-name: none` with the content still
  at `opacity: 1`. **10 checks, all green.** That last pair is the blank-panel guard *observed*
  rather than inferred, and it is the P1-2 lesson applied: a stylesheet that is reasoned about is
  not a stylesheet that has been run.
* **Not verified, and stated rather than glossed:** the 9 authenticated admin screens of
  `csp-check.mjs` did not run, because the local admin account is past its rotation gate. That is
  pre-existing state, confirmed in the local D1 (`must_change_password = 0`) rather than assumed —
  and chasing it is what found **P4-19**. None of those screens is touched by this change.
* **Files:** `frontend/src/index.css`, `components/motion/FadeIn.tsx`,
  `components/motion/FadeIn.test.tsx` (new), `features/student/pages/StudentAccessPage.tsx`,
  `backend/scripts/platform-gates.mjs`
* **Verify:** ✅ backend **927** · frontend **195 → 201** (24 files) · both type-checks · backend
  lint · `gate:platform` · `gate:bundle` (268 KiB) · `gate:assets` incl. the new join budget · build
  — all green.
...
```
        ...
        P4-2  password-reset email (Resend)     ✅ deployed 2026-08-01 — send witnessed on production;
                                                    the domain could not *receive* mail until Email
                                                    Routing + a catch-all closed the other half
        P4-15 Framer Motion off FadeIn          ✅ done 2026-08-01 — and off /, /login and /admin-login too
        P4-16 Zod off the join screen           ✅ done 2026-08-01 — /join 612 → 433 KiB (−29%)
        P4-19 csp-check could only pass once    ✅ done 2026-08-02 — fixed, pointed at production,
                                                    PASS 15/15 — and the first run found the CSP
                                                    had been blocking Web Analytics since 2026-07-28
NOW ──► Phase 4  (+ two dashboard-side steps left: DEPLOYMENT.md §8.1 WAF rule, §8.2 Workers Logs
        alert. §8.3 is done — and grew a §8.3.1 for the receiving side, which turned out to be
        the half that decided whether P4-2 worked at all)
```
...
| 2026-08-01 | Frontend flake — **two of them** | 927 BE / **195** FE | **One test failed once in nine runs during P4-2 and was never named, because the output was truncated by a `tail -8`.** Chasing it found two independent defects, and the second was hidden behind the first. **Method first, on the rule this document keeps re-earning: reproduce before fixing.** A capture harness (`npm run test:capture`, JSON report) went in *before* anything was diagnosed, and was itself proven by appending a deliberately failing test and confirming the report named file, test and message — because a green report and a missing report look identical, which is the P3-10 fault exactly. Then the suite was run under the condition it actually failed under: continuous backend-suite load. **It reproduced on run 01.** **Flake 1 — `CanonicalProgramPage › finds a merge target that is not on the current page`**, failing at **1,481 ms** on `Unable to find role="button" and name /merge bscs/i`, with the page shell rendered and the table not yet populated. The cause is structural and suite-wide: **testing-library's `asyncUtilTimeout` defaults to 1,000 ms and is a different budget from Vitest's `testTimeout`.** `vite.config.ts` records P3-3 raising `testTimeout` 5 s → 15 s precisely to survive a loaded machine, after four tests timed out — three in files that change did not touch. That was the right instinct at the wrong layer: a query that gives up after one second never reaches a fifteen-second test budget, so every one of the suite's ~dozens of `findBy*` calls kept its original ceiling and **the exposure P3-3 set out to close was never closed — only made invisible in the aggregate.** Fixed with `configure({ asyncUtilTimeout: 5_000 })` in `setupTests.ts`: ~3.4× the observed loss, and still 3× *under* `testTimeout`, so a genuinely stuck query fails with "Unable to find …" and a DOM dump — which names the problem — rather than an opaque test timeout, which does not. Proven to be a budget and not a blindfold: an unmatchable query still fails, at **5,027 ms**, with a real message. The explicit `{ timeout: 3000 }` in `QuestionWorkspace.test.tsx` went at the same time — once the default rose above it, that override had stopped being a raise and become the **tightest** budget in the suite, sitting on the one test that genuinely waits on wall-clock time. **Flake 2 — `QuestionWorkspace › auto-save coalesces typing into one request`**, which only surfaced on run 10 of the *next* campaign, once flake 1 stopped masking it. Not a timeout, and **not a product defect**: `clear()` opens the 800 ms `DEBOUNCE_MS` window, a loaded machine can spend longer than that finishing the typing, and the product then correctly saves the *empty* field — a pause is the signal the debounce exists to detect. `toHaveBeenCalledTimes(1)` was satisfied by that save, the wait returned, and the payload assertion read a request nobody was describing: `question_text: ""` where `"Reworded item."` was expected. **The test was measuring the clock.** Rewritten to wait on `toHaveBeenLastCalledWith` the final text — which is the stale-closure guard its own doc comment asks for — and to state coalescing as "fourteen keystrokes did not become fourteen requests" rather than pinning a count a legitimate extra save breaks. Proven by making the hook save on every keystroke: the coalescing assertion trips. **A dead end, recorded rather than left as a lurking suggestion:** fake timers were tried first, as the strongest fix available, and abandoned — freezing `setTimeout` deadlocks the render with *and* without `delay: null` on userEvent, and a test that hangs for fifteen seconds is a worse instrument than one that waits. **And the harness carried the P3-10 bug into its own first outing.** `node.exe` cannot resolve a git-bash POSIX path, so run 01 reported `failed=NOJSON` — and the loop treated "could not read the report" as "a test failed". It *coincided* with a genuine failure, which is the dangerous version of that conflation: the harness would have looked correct and been trusted. It now aborts loudly on an unreadable report instead. **The load hypothesis is now data rather than a guess**: `environment` runs ~86 s idle and 111–330 s under load, and the original unexplained failure sat at 118 s — inside the loaded range, above the idle baseline. CI gets the same capture: the frontend job runs `test:capture` and uploads the report, `.vitest-attachments/` and `__screenshots__` on `if: always()`, with `include-hidden-files: true` — because `upload-artifact@v4` skips dot-directories by default, which is the P3-10 fault verbatim, and because `extractText.test.ts` runs in real Chrome, where a failure writes the one piece of evidence that cannot be reconstructed after the runner is torn down. **And the verification campaign found a third, on run 14 of 20** — which is the argument for running it rather than declaring victory at two. `router.test.tsx › shows the shared loading state and then resolves the public group`, under the heaviest load of the whole exercise (`environment` **405 s** against ~86 s idle). **The failure said nothing**: `Error: Test timed out in 15000ms` over a stack of pure Vitest internals — no "Unable to find", no DOM dump, no clue which route had not resolved. The reason is a defect that has nothing to do with load: the file's own `RESOLVE = { timeout: 15_000 }` **equalled `testTimeout: 15_000`**, so the runner killed the test on the same tick the assertion budget expired, and *the query budget could never fire at all*. It is the mirror image of flake 1 — there an assertion budget sat far below the test budget and fired too early; here it sat level with it and was unreachable. **An assertion budget must sit strictly below the test budget or it cannot report.** The first attempt raised the file's ceiling to 30 s so the budget could fire — and **that fixed the message, not the flake**, which is worth recording because it was briefly written up as a fix. Under the same load the test still lost; it merely lost legibly. **The measurement that found the real fix also refuted the plan for it, twice.** With a cleared transform cache the first test costs **1,153 ms** while three later tests reaching already-compiled routes cost **33, 32 and 8 ms** — so the assertion was spending ~97% of its time measuring Vite's dev transform pipeline, which is not what it asserts, and compilation is exactly the term CPU contention inflates. The fix is to compile the three route groups in a `beforeAll` (60 s — **a hook may be generous precisely because it asserts nothing**, buying a bounded one-time cost rather than waiting on a condition that may never arrive). **Refutation 1:** that was predicted to take the test to ~30 ms; it took it to **392 ms**, and auth/access barely moved (327→341, 323→333) — because ~330 ms per group is React.lazy's own suspend-and-resolve cycle, which warming cannot touch, and the "fast" tests are fast only because earlier tests already resolved their groups. **Refutation 2, and the more useful one:** the plan also said to delete the file's 15 s `RESOLVE` budget as newly unnecessary. The arithmetic says otherwise — before, 1,153 ms inside 15 s needed **~13×** inflation to fail, which is precisely what run 14 delivered; warmed but inheriting the suite's 5 s, 392 ms needs **~12.8×**. *The same fragility, failing marginally sooner.* Halving the cost while cutting the budget by two thirds buys nothing, and the "obvious cleanup" would have shipped a no-op wearing the clothes of a fix. `RESOLVE` is restored at **10 s**: 392 ms is ~25× under it, and it stays below `testTimeout` so a genuine failure still reports. Both assertions proven live by mutation — an unfindable heading fails at **10,124 ms** with `TestingLibraryElementError: Unable to find role="heading" …`, and wrong fallback text fails at **107 ms**, which is the check that warming had not quietly collapsed the Suspense assertion into a tautology. **Verification at time of writing: campaign re-running after the fix; previous best was 19 of 20, with run 14 the failure now addressed.** Stated as it stands rather than rounded to "green" — and the honest scope is that the load applied here (a 927-test backend suite looping continuously alongside) is well beyond anything CI will do, so three fixes removing three real fragilities is not the same claim as a suite that cannot flake. What has actually changed is that the next one arrives **named**: the capture harness runs in CI on `if: always()`, so the next occurrence costs one run to diagnose instead of nine to notice. |
| 2026-08-01 | P4-2 (password-reset half) | **927** BE / 195 FE | **The item as written could not be built, and that was established before a line of code.** P4-2 said "email delivery"; Cloudflare Email *Sending* to arbitrary recipients requires Workers Paid, and the Free plan is not this project's current state but a **ratified requirement** (FULLPLAN §45) that `scripts/platform-gates.mjs` actively enforces — `wrangler.toml` says it outright: "The fix is not a paid plan." A first answer of "$5/mo, probably fine" was wrong for exactly the reason this document keeps recording: it was a confident claim about a constraint nobody had opened the file to read. Cloudflare's *free* path was then built — delivery to verified destination addresses — and **discarded on measurement**: the account had zero verified destinations, and the model requires a dashboard round-trip per staff mailbox, which is not self-service reset in any meaningful sense. **Shipped on Resend instead** (3,000/month, 100/day, one verified domain, any recipient), which leaves Cloudflare on the Free plan because it is external to the platform. **The item was also two items**, and only one was in scope: self-service reset is deviation D7, an acknowledged *gap*, while email *notifications* are §63 deferred-by-design behind a trigger — "in-app-only notifications are confirmed insufficient by real user feedback" — that has not fired two days after launch. `NotificationService` was left untouched, on the plan's own evidence rule rather than on appetite. **The real price is architectural and is now written down rather than absorbed**: Resend authenticates with an API key, so the system has its **first and only credential**, and `wrangler.toml`'s flat claim that "v1 has no secrets in steady state: every Cloudflare service is reached through a binding" stopped being true. That paragraph was rewritten to state the narrower truth and the trade that bought it, because P3-1 is this document's record of what a stale assertion costs when three files repeat it and nothing goes red. A new gate refuses `RESEND_API_KEY` in a `[vars]` block across **all four** wrangler configs — the one failure here is disclosure, and a committed key would still *work*, which is what lets it survive to a public repo. **The sharpest defect was in the security shape, and mutation caught it**: making the sender rethrow instead of swallowing turns `/auth/forgot-password` into a 500 **only for registered addresses** — an account-enumeration oracle rebuilt out of an error handler, undoing the generic acknowledgement §38 exists for. Four mutations were run against the Resend path (rethrow on refusal · rethrow on unreachable · key never reaches the Authorization header · sender sent bare instead of `Name <addr>`) and each failed the intended test with a real assertion message. **Two defects were found in the verification itself, both of the same family.** The three original platform gates were "fired red" with `perl` patterns containing `\n` against **CRLF** files, so two of three mutations never landed and the gates reported green — the P3-10 lesson arriving inside the step meant to apply it; redone asserting the byte count changed first. Then a *new* gate shipped red without being noticed until the gates were run: `!/RESEND_API_KEY/` is unanchored, so it matched the comment in `wrangler.test.toml` **explaining the gate** — a check that cannot distinguish a committed secret from a sentence about committed secrets. Deleted as redundant; the anchored assignment check already covered that file. Also: `@cloudflare/vitest-pool-workers` 0.18 exports no `fetchMock`, verified in its type definitions rather than assumed, so the seam is `vi.stubGlobal` — with the missing-key short-circuit as the hermeticity backstop, so a stub that failed to install still cannot reach `api.resend.com`. **Not witnessed**: no email has been observed arriving, because the domain is unverified and no key is set. DEPLOYMENT.md §8.3 has the three operator steps, including the grey-cloud DKIM trap and an explicit warning against `echo | wrangler secret put`, whose trailing newline is the P3-10 fault exactly. Also observed here and **chased down separately** — see the row above: the frontend suite failed one test on one run out of nine, and the P4-2 copy change was correctly ruled out as the cause (it is referenced by no test or script). Recording it rather than writing it off is what made it findable; it turned out to be **two** pre-existing flakes, neither related to this work. 927 BE (+12) · 195 FE · type-check · lint · gate:platform · gate:bundle (268 KiB) all green. |
| 2026-08-02 | P4-19 + P4-15/P4-16 **deployed** | 927 BE / 201 FE | **The bundle work went live, and the browser check reached all fifteen production screens for the first time — `PASS, 15 of 15`.** Two fixes had to land before that sentence was possible. `csp-check.mjs`'s admin leg was **single-use per database**: it required the §13.1 rotation, which clears the `must_change_password` it waits for, and rotated onto a random password it discarded — so every re-run after the first timed out, and an `if (signedIn)` silently skipped **nine screens** while the report said "6 screens checked" in the same tone it would have used for fifteen. The gate is optional now, a rotation is *printed* rather than discarded, and **coverage is itself an assertion** — the more expensive half, because it is the half that survives a green line. Then the production guard was **inverted rather than removed**: `--allow-production` must be passed deliberately, and the rotation is hard-refused at the branch, not merely unreachable. The old guard argued the CSP is a property of `_headers` and identical everywhere, so production offered nothing — sound about the policy, wrong about the system, and this document already held the counter-example in P3-1. **The very first production run failed all 15 screens and was right to.** Cloudflare injects the Web Analytics beacon at the *edge*, keyed off the User-Agent, so the local preview never emits it and a plain `curl` cannot even see it — the first confirmation attempt came back empty for exactly that reason. `script-src 'self'` refused it everywhere, so **Web Analytics had been enabled, injected and collecting nothing since the CSP shipped on 2026-07-28**, straight through the launch, harming nothing and therefore noticed by no one. **P1-2's webfont defect a second time**, found within one run of pointing a browser at the real domain. Fixed with one exact host in `script-src`; no `connect-src` needed, since the beacon reports to same-origin `/cdn-cgi/rum`. **And "no longer refused" was not accepted as "working"** — P4-2's `sent` trap applied before it could bite: a separate probe confirmed the beacon returns **200** and the RUM report **POSTs 204**. Deploy verified P3-1-style against the live domain rather than from wrangler's output: health `environment=production`, and the served `index-Cx6G3eaN.js` / `index-Dk-tWZl5.css` matching the local `dist/` hash for hash. Zod's accepted `eval` probe went **12 → 11 screens** across the deploy — one screen, `/join`, exactly what P4-16 aimed at. |
| 2026-08-01 | P4-15, P4-16 | 927 BE / **201** FE | **`/join` 612 → 433 KiB, a 29% cut to the only screen a student reaches before signing in to anything.** Both items hit the numbers their own lines predicted, and P4-15 paid out on **four** screens rather than the one it named — `FadeIn` has three consumers, so the landing page and both sign-in screens each lost the same 118 KiB. Framer Motion is not deleted, it is *relocated*: `QuestionWorkspace`'s drag-reorder still needs it and it now resolves into the builder chunk alone (637 → 636 KiB, 14 → 13 chunks). **The replacement CSS is written so that not animating is the finished state** — the keyframe declares only `from`, so the implicit `to` is the element's own computed style; the obvious spelling (`opacity: 0` on the base class, restored by the animation) leaves a permanently blank panel anywhere the animation does not run, which is worse than the bug being fixed. Reduced motion also got *more* correct: `useReducedMotion()` returned `null` on the first render and the truth on the second, so a fast paint animated at a user who had asked their OS not to. **P4-16's premise had to be measured, not assumed** — `schemas-*.js` was react-hook-form **and** Zod in one 88.7 KiB chunk, so the item only pays if rolldown splits them once `/join` stops needing Zod. It does: Zod is now alone at 61.9 KiB and absent from the join closure, react-hook-form's own `required`/`maxLength` rules replace the resolver, and **all 8 existing `StudentAccessPage` tests pass unchanged** — including P2-3's two `aria-describedby` assertions, which is the evidence the messages and their field wiring did not move. §38's generic-401 reasoning is why the ruleset was two required checks and two caps in the first place, so nothing was lost by dropping the schema. **A new gate, because the existing ones structurally could not see the regression**: one `import { z } from 'zod'` puts 61 KiB back and builds, type-checks and passes 201 tests, and `heaviest route cold load` will never catch it since `/join` is nowhere near the heaviest route. `--assets` now weighs `/join` against 460 KiB — proven by re-adding the import and watching the new gate go **red at 491 KiB while `heaviest route` stayed green at 662 ≤ 700**, the two numbers side by side being the argument for it. Every new assertion was fired red first by mutation (base-class `opacity`, an explicit `to` frame, the delay unit as `ms`, a dropped `className`, the media query flipped to `no-preference`), each via an exact-string edit that *errors* on a miss rather than a `perl` pattern that silently no-ops against CRLF — the P4-2 lesson applied in advance. **One defect found by running the gate rather than reading it**: `const join = …` shadowed `node:path`'s `join` in the same scope and put the manifest read into its TDZ. **Verified in real Chrome twice**: `csp-check.mjs` clean on all four affected screens (and Zod's accepted `eval` probe now fires on 2 screens, down from P3-3's 12 — the size win visible from a second instrument), plus a Playwright probe confirming `animation-name: fade-in-rise` actually *resolves on the elements*, settles at `opacity: 1`, and under `reducedMotion: 'reduce'` reports `none` with the content still visible — 10 checks, the blank-panel guard observed rather than inferred. **Stated rather than glossed**: csp-check's 9 authenticated screens did not run, because the local admin is past its rotation gate — confirmed in D1, not assumed, and chasing it found **P4-19**, that the script's admin leg destroys the state it depends on and so can only ever pass once per database. |
...
| 2026-08-01 | P4-2 **shipped** — Resend deployed, send witnessed | 927 BE / 195 FE | **Everything on the sending side was already done, and the feature was still at zero.** The domain was verified at Resend, DKIM and SPF were published, `RESEND_API_KEY` was set in *both* environments — and every reset link would have bounced, because `careerlinkai.online` had **no MX records at all**. Cloudflare Email Routing read `enabled: false`, `status: unconfigured`, zero destinations, catch-all disabled *and* set to `drop`. Production's only two staff accounts are `admin@` and `counselor@` on that domain, so the set of people who could use self-service password reset was **empty**. **The reason this was survivable long enough to reach a deploy is that it is invisible from inside the system.** Resend accepts the message, returns an id, and the Worker logs `password_reset_email.sent` — correctly, because from the sender's side it *was* sent. The bounce happens at a mail server this Worker never talks to. `sent` means "Resend accepted it", not "a human received it", and that gap is P3-10's fault restated: a green signal from the component that was working, standing in for an end-to-end claim nobody had tested. DEPLOYMENT.md §8.3 documented three operator steps and all three were about *sending*; the fourth and fifth did not exist. They do now, as §8.3.1. **Fixed with Email Routing, which is free** — unlike Email *Sending*, so §45 is untouched — and deliberately with a **catch-all** rather than two per-address rules: per-address would have covered the two seeded accounts and silently failed for every counselor created afterwards, through an admin UI, by someone who would never think to come back and add a routing rule. The catch-all is what makes the feature work for staff who do not exist yet, which was the explicit requirement. Verified by resolution rather than by dashboard: root `MX` now answers `route1/2/3.mx.cloudflare.net`, and **both** Resend records survived the change — `resend._domainkey` intact, `send.careerlinkai.online` still carrying `include:amazonses.com` and its SES return-path MX. The new root SPF that Email Routing adds does not touch Resend, and the reason is worth stating because it looks like it should: Resend's return-path is `send.careerlinkai.online`, so SPF is evaluated on that subdomain, while DKIM's `d=careerlinkai.online` aligns with the `From:` header. The zone's `A` records were untouched, so the site kept serving. **Two staff accounts were added at reachable mailboxes**, which needed a new script: `bootstrap-staff.mjs` writes *the two seed accounts* at fixed UUIDs and fixed addresses and should not learn how to do anything else. `scripts/create-staff.mjs` is the other job — one account, chosen email, PBKDF2 derived at run time in the **chained** 100k-round form `src/do/auth-guard.ts` uses, because Node has no per-call cap and a single 600k call yields a hash the deployed Worker cannot verify (which is what `bootstrap-staff.mjs` did on *its* first run). It defaults `must_change_password` to **0**, inverting the seed script, and the inversion is the point: there the password is generated and belongs to nobody, so §13.1's forced rotation is the activation step; here it is supplied by an operator who intends to keep it, and rotating it away would lock them out of the account they just made. **Two Windows-specific defects were found in that script by running it, not by reading it.** `npx` is a `.cmd` shim, so `execFileSync` needs `shell: true`, which hands argv back to `cmd` to re-split on whitespace — `--command "SELECT id, role FROM users …"` arrived as a dozen unknown arguments and wrangler exited on its own usage text, having never seen a query. Fixed by invoking `node node_modules/wrangler/bin/wrangler.js` directly, so `shell: false` passes argv verbatim. Then the workaround for *that* was itself wrong: routing the SELECT through `--file=` made wrangler treat it as a **batch ingest** and answer with a summary — `{"Total queries executed": 1, "Rows read": 0}` — instead of rows, so the existence check reported "no such user" *and* looked successful. Two opposite invocations for two opposite jobs, now stated as such in the script. `bootstrap-staff.mjs` met neither because it only ever had a file to pass. **Witnessed, and precisely bounded.** A real `POST /auth/forgot-password` on production logged `password_reset_email.sent` with Resend id `049f7ab2-4aa9-42d6-946a-466aec62b7d6` — a line a bad key, an unverified domain or a malformed `from` **cannot** produce, since each is a non-2xx the module reports as `rejected` with Resend's own sentence. So: key valid, domain verified at Resend, message accepted. `/reset-password` separately confirmed to serve the SPA on production, so the link is not a 404, and the login for both new accounts was verified against the live Worker via `--verify-url` — which is what proves the script's PBKDF2 and the Worker's agree, rather than proving the script is merely deterministic. **What is still not witnessed is stated as such**: a link arriving in an inbox and a human completing a reset. No log line in this system can make that claim, and §8.3.1 exists because the one that looks like it does — `sent` — is the trap. 927 BE · 195 FE · both type-checks · lint · platform gates all green before deploy. |
